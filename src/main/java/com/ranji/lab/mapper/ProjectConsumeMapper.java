...
public interface ProjectConsumeMapper {
    ...
    //查询全部
    @Select("select pc.*,ci.`name` consumeName from project_consume pc LEFT JOIN consume_inform ci on pc.experiment_consume_id = ci.id where pc.project_id = #{projectId} and status != 1")
    List<ProjectConsumeDto> projectIdFindAllProjectConsume(int projectId);
    //分类查询
    @Select("select pc.*,ci.`name` consumeName from project_consume pc LEFT JOIN consume_inform ci on pc.experiment_consume_id = ci.id where pc.project_id = #{projectId} and pc.status = #{status}")
    List<ProjectConsumeDto> typeProjectIdFindAllProjectConsume(int projectId,int status);
    //修改信息
